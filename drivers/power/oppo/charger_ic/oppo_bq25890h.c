...
static int bq25890h_set_vindpm_vol(struct oppo_chg_chip *chip, int vol);
int bq25890h_set_wdt_timer(struct oppo_chg_chip *chip, int reg);


static struct oppo_chg_chip *bq25890_chip;
...
static int __bq25890h_read_reg(struct oppo_chg_chip *chip, int reg, int *returnData)
{
    #ifdef 	CONFIG_OPPO_CHARGER_MTK
    #if defined CONFIG_OPPO_CHARGER_MTK6763  || defined(CONFIG_OPPO_CHARGER_MTK6771)
	int ret = 0;
...
static int bq25890h_read_reg(struct oppo_chg_chip *chip, int reg, int *returnData)
{
	int ret = 0;
...
static int __bq25890h_write_reg(struct oppo_chg_chip *chip, int reg, int val)
{
    #ifdef CONFIG_OPPO_CHARGER_MTK
    #if defined CONFIG_OPPO_CHARGER_MTK6763 || defined(CONFIG_OPPO_CHARGER_MTK6771)
	int ret = 0;
...
static int bq25890h_config_interface (struct oppo_chg_chip *chip, int RegNum, int val, int MASK)
{
    int bq25890h_reg = 0;
    int ret = 0;
...
static int bq25890h_input_current_limit_write(struct oppo_chg_chip *chip, int value)
{
	int rc = 0;
	int i = 0;
	int j = 0;
	...
	if(bq25890h_registers_read_full(chip)) {
		bq25890h_config_interface(chip, REG00_BQ25890H_ADDRESS, REG00_BQ25890H_INPUT_CURRENT_LIMIT_400MA, REG00_BQ25890H_INPUT_CURRENT_LIMIT_MASK);
		bq25890h_dump_registers(chip);
		printk("bq25890h_input_current_limit_write\n");
		return 0;
	}
	if(chip->charging_state == CHARGING_STATUS_FULL && chip->in_rechging == false){
		bq25890h_config_interface(chip, REG00_BQ25890H_ADDRESS, REG00_BQ25890H_INPUT_CURRENT_LIMIT_400MA, REG00_BQ25890H_INPUT_CURRENT_LIMIT_MASK);
		bq25890h_dump_registers(chip);
	...
	if (chg_vol <4300 && low_chg_vol_flag == 0 && !oppo_vooc_get_fastchg_started()) {
        rc = bq25890h_config_interface(chip, REG03_BQ25890H_ADDRESS,
		    REG03_BQ25890H_CHARGING_DISABLE, REG03_BQ25890H_CHARGING_MASK);
        if (rc < 0) {
    		chg_err("Couldn't bq25890h_disable_charging  rc = %d\n", rc);
    	}
        bq25890h_set_wdt_timer(chip, REG07_BQ25890H_I2C_WATCHDOG_TIME_DISABLE);
	    bq25890h_set_vindpm_vol(chip, 4500);
		bq25890h_suspend_charger(chip);
		low_chg_vol_flag = 1;
	}
	if (chg_vol >4400 && low_chg_vol_flag == 1) {
		bq25890h_enable_charging(chip);
		bq25890h_unsuspend_charger(chip);
		low_chg_vol_flag = 0;
	}
...
*/
	...
	j = 2; /* 500 */
	rc = bq25890h_config_interface(chip, REG00_BQ25890H_ADDRESS, REG00_BQ25890H_INPUT_CURRENT_LIMIT_500MA, REG00_BQ25890H_INPUT_CURRENT_LIMIT_MASK);
	msleep(90);
	chg_vol = battery_meter_get_charger_voltage();
	if (chg_vol < aicl_point_temp) {
		j = 2;
	...
	j = 3; /* 900 */
	rc = bq25890h_config_interface(chip, REG00_BQ25890H_ADDRESS, REG00_BQ25890H_INPUT_CURRENT_LIMIT_900MA, REG00_BQ25890H_INPUT_CURRENT_LIMIT_MASK);
	msleep(90);
	chg_vol = battery_meter_get_charger_voltage();
	if (chg_vol < aicl_point_temp) {
		j = j - 1;
	...
	aicl_point_temp = chip->sw_aicl_point;
	#endif
	rc = bq25890h_config_interface(chip, REG00_BQ25890H_ADDRESS, REG00_BQ25890H_INPUT_CURRENT_LIMIT_1500MA, REG00_BQ25890H_INPUT_CURRENT_LIMIT_MASK);
	msleep(90);
	chg_vol = battery_meter_get_charger_voltage();
	if (chg_vol < aicl_point_temp) {
	...
	aicl_point_temp = chip->sw_aicl_point;
	#endif
	rc = bq25890h_config_interface(chip, REG00_BQ25890H_ADDRESS, REG00_BQ25890H_INPUT_CURRENT_LIMIT_2000MA, REG00_BQ25890H_INPUT_CURRENT_LIMIT_MASK);
	msleep(90);
	#if defined(CONFIG_OPPO_CHARGER_MTK6763)  || defined(CONFIG_OPPO_CHARGER_MTK6771)
	for(i = 0; i < AICL_COUNT; i++){
		chg_vol = battery_meter_get_charger_voltage();
	...
	j = 7; /* 3000 */
	rc = bq25890h_config_interface(chip, REG00_BQ25890H_ADDRESS, REG00_BQ25890H_INPUT_CURRENT_LIMIT_3000MA, REG00_BQ25890H_INPUT_CURRENT_LIMIT_MASK);
	msleep(90);
	chg_vol = battery_meter_get_charger_voltage();
	if (chg_vol < aicl_point_temp) {
		j = j - 1;
...
aicl_pre_step:
	...
	switch (j) {
		case 0:
			rc = bq25890h_config_interface(chip, REG00_BQ25890H_ADDRESS, REG00_BQ25890H_INPUT_CURRENT_LIMIT_100MA, REG00_BQ25890H_INPUT_CURRENT_LIMIT_MASK);
		break;
		case 1:
			rc = bq25890h_config_interface(chip, REG00_BQ25890H_ADDRESS, REG00_BQ25890H_INPUT_CURRENT_LIMIT_150MA, REG00_BQ25890H_INPUT_CURRENT_LIMIT_MASK);
		break;
		case 2:
			rc = bq25890h_config_interface(chip, REG00_BQ25890H_ADDRESS, REG00_BQ25890H_INPUT_CURRENT_LIMIT_500MA, REG00_BQ25890H_INPUT_CURRENT_LIMIT_MASK);
		break;
		...
		break;
		case 4:
			rc = bq25890h_config_interface(chip, REG00_BQ25890H_ADDRESS, REG00_BQ25890H_INPUT_CURRENT_LIMIT_1200MA, REG00_BQ25890H_INPUT_CURRENT_LIMIT_MASK);
		break;
		case 5:
			rc = bq25890h_config_interface(chip, REG00_BQ25890H_ADDRESS, REG00_BQ25890H_INPUT_CURRENT_LIMIT_1500MA, REG00_BQ25890H_INPUT_CURRENT_LIMIT_MASK);
		break;
...
static int bq25890h_input_current_limit_write(struct oppo_chg_chip *chip, int value)
{
    ...
    rc = bq25890h_config_interface(chip, REG00_BQ25890H_ADDRESS, j << REG00_BQ25890H_INPUT_CURRENT_LIMIT_SHIFT, REG00_BQ25890H_INPUT_CURRENT_LIMIT_MASK);
    return rc;
}
#endif /*CONFIG_MTK_PMIC_CHIP_MT6353*/


static int bq25890h_charging_current_write_fast(struct oppo_chg_chip *chip, int chg_cur)
{
	int ret = 0;
	int value = 0;
	...
	ret = bq25890h_config_interface(chip, REG04_BQ25890H_ADDRESS, value,
...
static void bq25890h_set_aicl_point(struct oppo_chg_chip *chip, int vbatt)
{
	if (chip->hw_aicl_point == 4440 && vbatt > 4100) {
		chip->hw_aicl_point = 4500;
		chip->sw_aicl_point = 4550;
		bq25890h_set_vindpm_vol(chip, chip->hw_aicl_point);
...
static int bq25890h_set_enable_volatile_writes(struct oppo_chg_chip *chip)
{
    int rc = 0;
...
static int bq25890h_set_complete_charge_timeout(struct oppo_chg_chip * chip, int val)
{
    int rc = 0;
    ...
    rc = bq25890h_config_interface(chip, REG07_BQ25890H_ADDRESS,
...
int bq25890h_float_voltage_write(struct oppo_chg_chip *chip, int vfloat_mv)
{
	int rc = 0;
    int value = 0;
	...
	rc = bq25890h_config_interface(chip, REG06_BQ25890H_ADDRESS, value, REG06_BQ25890H_CHARGING_VOL_LIMIT_MASK);
	return rc;
}

int bq25890h_set_prechg_current(struct oppo_chg_chip *chip, int ipre_mA)
{
    int value = 0;
	...
	return bq25890h_config_interface(chip, REG05_BQ25890H_ADDRESS,
		value, REG05_BQ25890H_PRE_CHARGING_CURRENT_LIMIT_MASK);
}

static int bq25890h_set_termchg_current(struct oppo_chg_chip *chip, int term_curr)
{
	int value = 0;
	int rc = 0;
	...
	bq25890h_config_interface(chip, REG05_BQ25890H_ADDRESS,
		value, REG05_BQ25890H_TERM_CHARGING_CURRENT_LIMIT_MASK);
	return rc;
}

int bq25890h_set_rechg_voltage(struct oppo_chg_chip *chip, int recharge_mv)
{
	int reg = 0;
	int rc = 0;
    ...
    rc = bq25890h_config_interface(chip, REG06_BQ25890H_ADDRESS, reg, REG06_BQ25890H_RECHARGING_THRESHOLD_VOL_MASK);
    if (rc) {
...
static int bq25890h_set_chging_term_disable(struct oppo_chg_chip *chip)
{
	int rc = 0;
	if (atomic_read(&chip->charger_suspended) == 1) {
		return 0;
...
int bq25890h_kick_wdt(struct oppo_chg_chip *chip)
{
	int rc = 0;
...
static bool bq25890h_check_charger_suspend_enable(struct oppo_chg_chip *chip);
int bq25890h_enable_charging(struct oppo_chg_chip *chip)
{
	int rc = 0;
	...
	if (bq25890h_check_charger_suspend_enable(chip)) {
		bq25890h_unsuspend_charger(chip);
	}
...
int bq25890h_disable_charging(struct oppo_chg_chip *chip)
{
	int rc = 0;
...
int bq25890h_enable_charging(struct oppo_chg_chip *chip)
{
	...
	bq25890h_set_wdt_timer(chip, REG07_BQ25890H_I2C_WATCHDOG_TIME_40S);
...
int bq25890h_disable_charging(struct oppo_chg_chip *chip)
{
	...
	{
		bq25890h_input_current_limit_write(chip, 400);
	}
...
static bool bq25890h_check_charger_suspend_enable(struct oppo_chg_chip *chip)
{
	...
	rc = bq25890h_read_reg(chip, REG00_BQ25890H_ADDRESS, &reg_val);
	if (rc) {
...
static int bq25890h_check_charging_enable(struct oppo_chg_chip *chip)
{
	bool rc = 0;

	rc = bq25890h_check_charger_suspend_enable(chip);
	if(rc)
		return 0;
	else
		return 1;
...
static int bq25890h_check_charging_enable(struct oppo_chg_chip *chip)
{
	...
	rc = bq25890h_read_reg(chip, REG03_BQ25890H_ADDRESS, &reg_val);
    if (rc) {
...
int bq25890h_registers_read_full(struct oppo_chg_chip *chip)
{
	int rc = 0;
    int reg_full = 0;
	int reg_ovp = 0;
    ...
    rc = bq25890h_read_reg(chip, REG0B_BQ25890H_ADDRESS, &reg_full);
    if (rc) {
	...
	rc = bq25890h_read_reg(chip, REG0C_BQ25890H_ADDRESS, &reg_ovp);
	if (rc) {
...
int bq25890h_otg_enable(void)
{
	...
	if (atomic_read(&bq25890_chip->charger_suspended) == 1) {
		return 0;
	}
	#endif /* CONFIG_OPPO_CHARGER_MTK */
	bq25890h_config_interface(bq25890_chip, REG00_BQ25890H_ADDRESS,
		REG00_BQ25890H_SUSPEND_MODE_DISABLE, REG00_BQ25890H_SUSPEND_MODE_MASK);

	rc = bq25890h_config_interface(bq25890_chip, REG03_BQ25890H_ADDRESS,
	...
	oppo_otg_online = 1;
	bq25890h_set_wdt_timer(bq25890_chip, REG07_BQ25890H_I2C_WATCHDOG_TIME_40S);
...
int bq25890h_otg_disable(void)
{
	...
	bq25890h_set_wdt_timer(bq25890_chip, REG07_BQ25890H_I2C_WATCHDOG_TIME_DISABLE);
	oppo_otg_online = 0;
	oppo_chg_set_otg_online(false);
	bq25890h_read_reg(bq25890_chip, REG0C_BQ25890H_ADDRESS, &val_buf);
	bq25890h_read_reg(bq25890_chip, REG0C_BQ25890H_ADDRESS, &val_buf);
	return rc;
}

int bq25890h_suspend_charger(struct oppo_chg_chip *chip)
{
	int rc = 0;
	...
	rc = bq25890h_config_interface(chip, REG00_BQ25890H_ADDRESS,
...
int bq25890h_unsuspend_charger(struct oppo_chg_chip *chip)
{
	int rc = 0;
...
#if defined CONFIG_MTK_PMIC_CHIP_MT6353 || defined(CONFIG_OPPO_CHARGER_MTK6763) || defined(CONFIG_OPPO_CHARGER_MTK6771)
int bq25890h_reset_charger(struct oppo_chg_chip *chip)
{
	int rc = 0;
	...
	rc = bq25890h_config_interface(chip, REG00_BQ25890H_ADDRESS, 0x08, 0xFF);
	if (rc < 0) {
		chg_err("Couldn't reset REG00 rc = %d\n", rc);
	}
	rc = bq25890h_config_interface(chip, REG01_BQ25890H_ADDRESS, 0x01, 0xFF);
	if (rc < 0) {
		chg_err("Couldn't reset REG01 rc = %d\n", rc);
	}
	rc = bq25890h_config_interface(chip, REG02_BQ25890H_ADDRESS, 0x00, 0xFF);
	if (rc < 0) {
		chg_err("Couldn't reset REG02 rc = %d\n", rc);
	}
	rc = bq25890h_config_interface(chip, REG03_BQ25890H_ADDRESS, 0x1A, 0xFF);
	if (rc < 0) {
		chg_err("Couldn't reset REG03 rc = %d\n", rc);
	}
	rc = bq25890h_config_interface(chip, REG04_BQ25890H_ADDRESS, 0x08, 0xFF);
	if (rc < 0) {
		chg_err("Couldn't reset REG04 rc = %d\n", rc);
	}
	rc = bq25890h_config_interface(chip, REG05_BQ25890H_ADDRESS, 0x13, 0xFF);
	if (rc < 0) {
		chg_err("Couldn't reset REG05 rc = %d\n", rc);
	}
	rc = bq25890h_config_interface(chip, REG06_BQ25890H_ADDRESS, 0x86, 0xFF);
	if (rc < 0) {
		chg_err("Couldn't reset REG06 rc = %d\n", rc);
	}
	rc = bq25890h_config_interface(chip, REG07_BQ25890H_ADDRESS, 0x5D, 0xFF);
	if (rc < 0) {
		chg_err("Couldn't reset REG07 rc = %d\n", rc);
	}
	rc = bq25890h_config_interface(chip, REG08_BQ25890H_ADDRESS, 0x03, 0xFF);
	if (rc < 0) {
		chg_err("Couldn't reset REG08 rc = %d\n", rc);
	}
		rc = bq25890h_config_interface(chip, REG09_BQ25890H_ADDRESS, 0x40, 0xFF);
	if (rc < 0) {
		chg_err("Couldn't reset REG09 rc = %d\n", rc);
	}
	rc = bq25890h_config_interface(chip, REG0A_BQ25890H_ADDRESS, 0x73, 0xFF);
	if (rc < 0) {
		chg_err("Couldn't reset REG0A rc = %d\n", rc);
	}
	rc = bq25890h_config_interface(chip, REG0D_BQ25890H_ADDRESS, 0x91, 0xFF);
	if (rc < 0) {
		chg_err("Couldn't reset REG0D rc = %d\n", rc);
	}
	rc = bq25890h_config_interface(chip, REG14_BQ25890H_ADDRESS, 0x5F, 0xFF);
	if (rc < 0) {
...
int bq25890h_reset_charger(struct oppo_chg_chip *chip)
{
    ...
    rc = bq25890h_config_interface(chip, REG01_BQ25890H_ADDRESS,
...
static bool bq25890h_check_charger_resume(struct oppo_chg_chip *chip)
{
	if (atomic_read(&chip->charger_suspended) == 1) {
		return false;
	} else {
...
int bq25890h_dump_registers(struct oppo_chg_chip *chip)
{
	int rc = 0;
	int addr = 0;
	...
	for (addr = BQ25890H_FIRST_REG; addr <= BQ25890H_LAST_REG; addr++) {
		rc = bq25890h_read_reg(chip, addr, &val_buf[addr]);
		if (rc) {
...
static int bq25890h_get_chg_current_step(struct oppo_chg_chip *chip)
{
	int rc = 0;
	int reg_val = 0;

	rc = bq25890h_read_reg(chip, REG02_BQ25890H_ADDRESS, &reg_val);
	if (rc) {
...
int bq25890h_hardware_init(struct oppo_chg_chip *chip)
{
	/* must be before set_vindpm_vol and set_input_current */
	chip->hw_aicl_point = 4440;
	chip->sw_aicl_point = 4500;

	bq25890h_reset_charger(chip);

	if (get_boot_mode() == KERNEL_POWER_OFF_CHARGING_BOOT) {
		#ifndef CONFIG_MTK_PMIC_CHIP_MT6353
		bq25890h_disable_charging(chip);
		#endif /* CONFIG_MTK_PMIC_CHIP_MT6353 */
		bq25890h_float_voltage_write(chip, 4400);
		msleep(100);
	}
	#if defined(CONFIG_OPPO_CHARGER_MTK6763) || defined(CONFIG_OPPO_CHARGER_MTK6771)
	bq25890h_float_voltage_write(chip, 4370);
	#else
	bq25890h_float_voltage_write(chip, 4320);
	#endif
	
	bq25890h_set_enable_volatile_writes(chip);

	bq25890h_set_complete_charge_timeout(chip, OVERTIME_DISABLED);

    bq25890h_set_prechg_current(chip, 300);

	bq25890h_charging_current_write_fast(chip, 512);

    bq25890h_set_termchg_current(chip, 150);

    bq25890h_set_rechg_voltage(chip, 100);
...
void is_iindpm_mode(struct oppo_chg_chip *chip)
{
	int rc = 0;
	int val_buf;
	int reg_ovp;
	int val_buf1;
	rc = bq25890h_read_reg(chip, REG13_BQ25890H_ADDRESS, &reg_ovp);
	val_buf = reg_ovp & 0x40;
	rc = bq25890h_read_reg(chip, REG0B_BQ25890H_ADDRESS, &reg_ovp);
...
//static DEVICE_ATTR(bq25890h_access, 0664, show_bq25890h_access, store_bq25890h_access);
#ifdef VENDOR_EDIT
extern CHARGER_TYPE MTK_CHR_Type_num;
extern unsigned int upmu_get_rgs_chrdet(void);

CHARGER_TYPE mt_charger_type_detection_bq25890h(void)
{
	int rc =0;
	int addr = 0;
	int val_buf;
	...
	int i = 0;
	if (!bq25890_chip) {
		pr_err("%s bq25890_chip null,return\n", __func__);
	...
	//bq25890h_config_interface(bq25890_chip, REG00_BQ25890H_ADDRESS, 0, 0x80);
	bq25890h_config_interface(bq25890_chip, REG02_BQ25890H_ADDRESS, 0x01, 0);
	bq25890h_config_interface(bq25890_chip, REG02_BQ25890H_ADDRESS, 0x02, 0);
	bq25890h_dump_registers(bq25890_chip);
	
	rc = bq25890h_read_reg(bq25890_chip, REG02_BQ25890H_ADDRESS, &val_buf);
	val_buf &=0x02;
	while (val_buf == 0x02 && count <20) {
		...
		if (!upmu_get_rgs_chrdet()) {
			MTK_CHR_Type_num = CHARGER_UNKNOWN;
            bq25890h_config_interface(bq25890_chip, REG02_BQ25890H_ADDRESS, 0, 0xff);
			return MTK_CHR_Type_num;
		}
	}
	if (count == 20) {
	...
	rc = bq25890h_read_reg(bq25890_chip, addr, &val_buf);
...
{	
	...
	printk("do_bq25890h_irq_delay_work  flag =%d\n",otg_overcurrent_flag);
...
static int bq25890h_irq_registration(struct oppo_chg_chip *chip)
{
	int ret = 0;
...
static int bq25890h_driver_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	reg = bq25890h_dump_registers(chip);
	if (reg < 0) {
		return 0;
	}
	charger_ic_flag = 1;
	if (is_project(OPPO_17197) || is_project(OPPO_18311)) {
...
static int bq25890h_resume(struct device *dev)
{
	...
	atomic_set(&bq25890_chip->charger_suspended, 0);
...
static int bq25890h_suspend(struct device *dev)
{
	...
	atomic_set(&bq25890_chip->charger_suspended, 1);
...
static void bq25890h_reset(struct i2c_client *client)
{
	bq25890h_otg_disable();
	if(bq25890_chip!= NULL){
		enter_ship_mode_function(bq25890_chip);
	}
	else
		printk("enter_ship_mode_function fail_bq25890\n");
