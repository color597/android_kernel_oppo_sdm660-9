...
{
	return strncmp(t1, t2, TASK_COMM_LEN);
}

static int pid_node_tree_insert(struct pid_node *data, struct rb_root *root)
{
...
static int pid_stat_tree_insert(struct pid_stat *data, struct rb_root *root)
{
	return pid_node_tree_insert(&data->tn, root);
}
...
static struct pid_stat *create_pid_stat(struct tag_stat *tag_entry, char *comm, pid_t pid)
{
	...
	if (pid_stat_tree_insert(new_pid_stat_entry, &tag_entry->pid_stat_tree)) {
		kfree(new_pid_stat_entry);
		return NULL;
	}
